...
def main_ckpt(_):
  model_folder = os.path.join(cfg.dataset_params['model_path'], FLAGS.net,
                              'ckpt')
...
def main_pb(_):
  ...
  for i in range(0, len(images), 50):
    ...
    correct_pred = tf.equal(tf.argmax(predict, 1, output_type=tf.int32),
                            labels[i:i + 50])
    acc = tf.reduce_sum(tf.cast(correct_pred, tf.float32))
    total_count += sess1.run(acc)
    accuracy = total_count / (i + 1)
    print("test samples:%d,accuracy:%d/%d = %.4f " % (
      i + 1, total_count, i + 1, accuracy))
