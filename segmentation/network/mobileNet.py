...
class MobileNet(object):
    ...
    def __init__(self, is_training=True, keep_prob=0.5, num_classes=10):
        ...
        self.regularizer = tf.contrib.layers.l2_regularizer(
            scale=self.weight_decay)
        self.initializer = tf.contrib.layers.xavier_initializer()
        self.keep_prob = keep_prob

    def conv2d(self, inputs, out_channel, kernel_size=1, stride=1):
        inputs = tf.layers.conv2d(inputs, filters=out_channel,
        ...
        inputs = tf.layers.batch_normalization(inputs,
                                               ...
                                               name='bn' + str(self.conv_num))
        self.conv_num += 1
        return tf.nn.relu(inputs)

    def _variable_with_weight_decay(self, name, shape, wd):
        var = tf.get_variable(name, shape, initializer=self.initializer,
                              dtype=tf.float32)
        if wd is not None:
    ...
    def depthwise_conv2d(self, inputs, kernel_size, stride=1):
        ...
        with tf.variable_scope(scope) as scope:
            kernel = self._variable_with_weight_decay('weight',
                                                      shape=kernel_size,
                                                      wd=self.weight_decay)
            depthwise = tf.nn.depthwise_conv2d(inputs, kernel,
                                               [1, stride, stride, 1],
                                               padding='SAME')
            biases = tf.get_variable('biases', depthwise.shape[3],
    ...
    def separable_conv2d(self, inputs, out_channel, kernel_size, stride=1):
        inputs = self.depthwise_conv2d(inputs, kernel_size=kernel_size,
                                       stride=stride)
    ...
    def forward(self, inputs):
        ...
        out = self.separable_conv2d(out, out_channel=64,
                                    kernel_size=[3, 3, 32, 1], stride=1)
        out = self.separable_conv2d(out, out_channel=128,
                                    kernel_size=[3, 3, 64, 1], stride=2)
        out = self.separable_conv2d(out, out_channel=128,
                                    kernel_size=[3, 3, 128, 1], stride=1)
        out = self.separable_conv2d(out, out_channel=256,
                                    kernel_size=[3, 3, 128, 1], stride=2)
        out = self.separable_conv2d(out, out_channel=256,
                                    kernel_size=[3, 3, 256, 1], stride=1)
        out = self.separable_conv2d(out, out_channel=512,
                                    kernel_size=[3, 3, 256, 1], stride=2)
        out = self.make_layer(out, repeat=5)
        out = self.separable_conv2d(out, out_channel=1024,
                                    kernel_size=[3, 3, 512, 1], stride=2)
        out = self.separable_conv2d(out, out_channel=1024,
                                    kernel_size=[3, 3, 1024, 1], stride=1)
    ...
    def make_layer(self, inputs, repeat=5):
        for i in range(repeat):
            inputs = self.separable_conv2d(inputs, out_channel=512,
                                           kernel_size=[3, 3, 512, 1], stride=1)
        return inputs
