...
class Resnet(object):
    ...
    def residual_block(self, inputs, output_channel, stride=[1, 1]):
        ...
        inputs = self.conv2d(inputs, output_channel, stride=stride[1],
                             relu=False)
        inputs_width = inputs.shape[-2]
        if input_channel != output_channel or x_width != inputs_width:
            residual = self.conv2d(residual, output_channel, kernel_size=1,
                                   stride=stride[1], relu=False)
        return tf.nn.relu(tf.add(inputs, residual))

    def conv2d(self, inputs, output_channel, kernel_size=3, stride=1,
               relu=True):
        inputs = tf.layers.conv2d(inputs, filters=output_channel,
    ...
    def make_layer(self, inputs, output_channel):
        ...
        inputs = self.residual_block(inputs, output_channel[1],
                                     stride=[1, stride_2])
        return inputs
