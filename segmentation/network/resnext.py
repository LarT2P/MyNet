...
class resNext(object):
    ...
    def bottleneck_layer(self, inputs, out_channel, stride):
        ...
        out = self.conv2d(inputs, out_channel=out_channel[0], kernel_size=1,
                          stride=stride[0])
        out = self.group_conv2d(out, out_channel=out_channel[1],
                                stride=stride[1])
        out = self.conv2d(out, out_channel=out_channel[2], kernel_size=1,
                          stride=stride[2], relu=False)

        out_width = out.shape[-2]
        if input_channel != out_channel[2] or input_width != out_width:
            residual = self.conv2d(residual, out_channel[2], kernel_size=1,
                                   stride=stride[2], relu=False)
    ...
    def group_conv2d(self, inputs, out_channel, stride):
        ...
        with tf.variable_scope(scope) as scope:
            filters = self.variable_with_weight_deacy('weights',
                                                      [3, 3, in_channel,
                                                       out_channel],
            ...
            for index in range(self.cardinality):
                out = tf.nn.conv2d(inputs[:, :, :,
                                   index * in_channel:(index + 1) * in_channel],
                                   filter=filters[:, :, :,
                                          index * out_subchannel:(
                                                                             index + 1) * out_subchannel],
                                   strides=[1, stride, stride, 1],
                                   padding='SAME')
                group_out.append(out)
            group_out = tf.concat(group_out, axis=-1,
                                  name='conv_' + str(self.conv_num))
            out = tf.layers.batch_normalization(group_out,
    ...
    def conv2d(self, inputs, out_channel, kernel_size, stride, relu=True):
        out = tf.layers.conv2d(inputs, filters=out_channel,
    ...
    def forward(self, inputs):
        ...
        out = tf.layers.max_pooling2d(out, pool_size=3, strides=2,
                                      padding='same', name='maxpool_0')
        out = self.make_layer(out, [128, 128, 256], self.block_nums[0],
                              downsample=False)
        ...
        out = tf.layers.average_pooling2d(out, pool_size=(pool_size, pool_size),
    ...
    def make_layer(self, inputs, output_channel, block_num, downsample=True):
        ...
        for num in range(block_num - 1):
            inputs = self.bottleneck_layer(inputs, out_channel=output_channel,
                                           stride=[1, 1, 1])
        inputs = self.bottleneck_layer(inputs, out_channel=output_channel,
                                       stride=[1, 1, stride_2])
        return inputs
